    ...
    public static class NetworkComms
    {
        ...
        internal static void CompleteIncomingItemTask(object priorityQueueItemObj)
        {
            ...
            try
            {
                ...
                if (item.PacketHeader.PacketType == Enum.GetName(typeof(ReservedPacketType), ReservedPacketType.np))
                {
                    if (NetworkComms.LoggingEnabled) NetworkComms.Logger.Trace("Unwrapping a " + ReservedPacketType.np + " packet from " + item.Connection.ConnectionInfo + " with a priority of " + item.Priority.ToString() + ".");
