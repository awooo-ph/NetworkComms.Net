        ...
        public static string LogException(Exception ex, string fileName, string optionalCommentStr = "")
        {
            ...
            lock (errorLocker)
            {
                ...
                using (Process currentProcess = System.Diagnostics.Process.GetCurrentProcess())
                    entireFileName = "./Logs/"+ fileName + " " + DateTime.Now.ToString("HH.mm.ss.fff") + " " + DateTime.Now.ToString("dd-MM-yyyy" + " [" + currentProcess.Id.ToString() + "-" + Thread.CurrentThread.ExecutionContext + "]");
#endif
