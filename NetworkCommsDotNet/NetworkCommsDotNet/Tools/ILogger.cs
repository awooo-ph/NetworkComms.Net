...
namespace NetworkCommsDotNet.Tools
{
    ...
    public class LiteLogger : ILogger
    {
        ...
        /// <inheritdoc />
        public void Trace(string message){log((int)LogLevels.Trace, message);}

        /// <inheritdoc />
        public void Debug(string message) {log((int)LogLevels.Debug, message); }

        /// <inheritdoc />
        public void Fatal(string message) { log((int)LogLevels.Fatal, message); }

        /// <inheritdoc />
        public void Fatal(string message, Exception ex) { log((int)LogLevels.Fatal, message); }

        /// <inheritdoc />
        public void Info(string message) {log((int)LogLevels.Info, message); }

        /// <inheritdoc />
        public void Warn(string message) {log((int)LogLevels.Warn, message); }

        /// <inheritdoc />
        public void Error(string message) { log((int)LogLevels.Error, message); }

        public void Log(string message){log(7,message);}

        private void log(int level, string message)
        {
            var v = (int) LogLevel;
            if ((int)level < v) return;
            ...
            if (_currentLogMode == LogMode.ConsoleAndLogFile || _currentLogMode == LogMode.ConsoleOnly)
            {
                ...
                switch (level)
                {
                    ...
                    case (int)LogLevels.Debug:
                        ...
                        break;
                    case (int)LogLevels.Trace:
                        Console.ForegroundColor = ConsoleColor.DarkGray;
                        break;
                    case (int)LogLevels.Fatal:
                ...
                var l = Enum.GetName(typeof(LogLevels), level);

                Console.WriteLine(level == 7 ? message : $"[{l}] {message}");
                Console.ResetColor();
            }
#endif
            ...
            if ((_currentLogMode == LogMode.ConsoleAndLogFile || _currentLogMode == LogMode.LogFileOnly) && LogFileLocationName != null)
            {
                try
                {
                    lock (_locker)
                    {
                        ...
                        writeTask.Wait(); 
#else
                        var folder = Path.GetDirectoryName(LogFileLocationName);
                        if (folder != "Logs")
                        {
                            if (!(Directory.Exists("./Logs")))
                                Directory.CreateDirectory("./Logs");
                            LogFileLocationName = Path.Combine(".", "Logs", Path.GetFileName(LogFileLocationName));
                        }
